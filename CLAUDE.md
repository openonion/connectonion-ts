...
4. **Session**: Two-layer architecture
   ...
   - **React hook** (`useAgentForHuman()`, in [`@connectonion/react`](https://github.com/openonion/connectonion-react)): Auto-persists to localStorage via Zustand, keyed by `co:agent:{address}:session:{sessionId}`
...
- **Session & trace**: In-memory for local Agent; RemoteAgent syncs session from server on each streaming event. `@connectonion/react`'s `useAgentForHuman` hook adds localStorage persistence via Zustand
...
- **React hook** (`useAgentForHuman()`, a separate package — `@connectonion/react`): Session auto-persists to localStorage via Zustand by `sessionId`. Survives browser refresh. Call `reset()` to clear.
